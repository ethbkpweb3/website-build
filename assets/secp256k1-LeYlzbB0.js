...
function B(t, e, r) {
    ...
    if ("string" == typeof e) try {
        ...
        if (!l(e)) throw new Error(`${t} must be hex string or Uint8Array`);
        n = Uint8Array.from(e)
    }
    const i = n.length;
    if ("number" == typeof r && i !== r) throw new Error(`${t} expected ${r} bytes, got ${i}`);
    return n
}
...
function O(t, e, r) {
    ...
    const s = () => {
        ...
        f = (...t) => r(i, n, ...t),
        a = (t = S()) => {
            i = f(A([0]), t), n = f(), 0 !== t.length && (i = f(A([1]), t), n = f())
        },
        u = () => {
...
function R(t, e, r = {}) {
    const n = (e, r, n) => {
        const i = q[r];
        if ("function" != typeof i) throw new Error(`Invalid validator "${r}", expected function`);
        const o = t[e];
        if (!(n && void 0 === o || i(o, t))) throw new Error(`Invalid param ${String(e)}=${o} (${typeof o}), expected ${r}`)
    };
...
function Z(t, e, r) {
    if (r <= H || e < H) throw new Error("Expected power/modulo > 0");
    if (r === T) return H;
    let n = T;
    for (; e > H;) e & T && (n = n * t % r), t = t * t % r, e >>= T;
    return n
}
...
function j(t, e) {
    if (t === H || e <= H) throw new Error(`invert: expected positive integers, got n=${t} mod=${e}`);
    let r = F(t, e),
        n = e,
        i = H,
        o = T;
    ...
    if (n !== T) throw new Error("invert: does not exist");
    return F(i, e)
}
...
function C(t) {
    ...
    if (t % P === z) {
        ...
        return function(t, r) {
            ...
            if (!t.eql(t.sqr(f), r)) throw new Error("Cannot find square root");
            return f
        }
    }
    return function(t) {
        ...
        return function(t, s) {
            ...
            for (; !t.eql(c, t.ONE);) {
                ...
                let e = 1;
                for (let n = t.sqr(c); e < f && !t.eql(n, t.ONE); e++) n = t.sqr(n);
                const r = t.pow(a, T << BigInt(f - e - 1));
                a = t.sqr(r), u = t.mul(u, r), c = t.mul(c, a), f = e
            }
            return u
        }
    }(t)
...
function W(t) {
    return R(t.Fp, V.reduce(((t, e) => (t[e] = "function", t)), {
        ORDER: "bigint",
        MASK: "bigint",
        BYTES: "isSafeInteger",
        BITS: "isSafeInteger"
    ...
    }), Object.freeze({ ..._(t.n, t.nBitLength),
        ...t,
        p: t.Fp.ORDER
    })
}
...
function rt(t) {
    ...
    function a(t) {
        ...
        if (r && "bigint" != typeof t) {
            if (t instanceof Uint8Array && (t = d(t)), "string" != typeof t || !r.includes(t.length)) throw new Error("Invalid key");
            t = t.padStart(2 * n, "0")
        }
        let s;
        try {
    ...
    class l {
        ...
        assertValidity() {
            ...
            const i = r.sqr(n),
                s = o(t);
            if (!r.eql(i, s)) throw new Error("bad point: equation left != right");
        ...
        equals(t) {
            ...
            const {
                ...
                py: s,
                pz: f
            } = t, a = r.eql(r.mul(e, f), r.mul(o, i)), u = r.eql(r.mul(n, f), r.mul(s, i));
            return a && u
        }
        negate() {
        ...
        double() {
            ...
            let a = r.ZERO,
                ...
                g = r.mul(f, f),
                w = r.mul(o, s);
            return w = r.add(w, w), c = r.mul(o, f), c = r.add(c, c), a = r.mul(t, c), u = r.mul(i, g), u = r.add(a, u), a = r.sub(d, u), u = r.add(d, u), u = r.mul(a, u), a = r.mul(w, a), c = r.mul(i, c), g = r.mul(t, g), w = r.sub(h, g), w = r.mul(t, w), w = r.add(w, c), c = r.add(h, h), h = r.add(c, h), h = r.add(h, g), h = r.mul(h, w), u = r.add(u, h), g = r.mul(s, f), g = r.add(g, g), h = r.mul(g, w), a = r.sub(a, h), c = r.mul(g, d), c = r.add(c, c), c = r.add(c, c), new l(a, u, c)
        }
        add(t) {
            c(t);
            const {
            ...
            let p = r.mul(n, s),
                ...
                E = r.add(n, i),
                b = r.add(s, f);
            E = r.mul(E, b), b = r.add(p, y), E = r.sub(E, b), b = r.add(n, o);
            let B = r.add(s, a);
            return b = r.mul(b, B), B = r.add(p, m), b = r.sub(b, B), B = r.add(i, o), u = r.add(f, a), B = r.mul(B, u), u = r.add(y, m), B = r.sub(B, u), d = r.mul(g, b), u = r.mul(w, m), d = r.add(u, d), u = r.sub(y, d), d = r.add(y, d), h = r.mul(u, d), y = r.add(p, p), y = r.add(y, p), m = r.mul(g, m), b = r.mul(w, b), y = r.add(y, m), m = r.sub(p, m), m = r.mul(g, m), b = r.add(b, m), p = r.mul(y, b), h = r.add(h, p), p = r.mul(B, b), u = r.mul(E, u), u = r.sub(u, p), p = r.mul(E, y), d = r.mul(B, d), d = r.add(d, p), new l(u, h, d)
        }
        subtract(t) {
        ...
        multiplyUnsafe(t) {
            ...
            let {
                ...
                k2neg: a,
                k2: u
            } = i.splitScalar(t), c = n, h = n, d = this;
            for (; s > X || u > X;) s & tt && (c = c.add(d)), u & tt && (h = h.add(d)), d = d.double(), s >>= tt, u >>= tt;
            return o && (c = c.negate()), a && (h = h.negate()), h = new l(r.mul(h.px, i.beta), h.py, h.pz), c.add(h)
        }
        multiply(t) {
            f(t);
            let n, i, o = t;
            ...
            if (s) {
                ...
                u = g.constTimeNegate(t, u), h = g.constTimeNegate(f, h), h = new l(r.mul(h.px, s.beta), h.py, h.pz), n = u.add(h), i = c.add(d)
            } else {
                const {
                    p: t,
                    f: e
        ...
        g = function(t, e) {
            ...
            return {
                ...
                unsafeLadder(e, r) {
                    let n = t.ZERO,
                        i = e;
                    for (; r > Y;) r & M && (n = n.add(i)), i = i.double(), r >>= M;
                    return n
                },
...
function nt(t) {
    ...
    const {
        ...
        fromBytes(t) {
            ...
            if (e !== i || 2 !== n && 3 !== n) {
                ...
                const t = y(s);
                if (!(X < (f = t) && f < r.ORDER)) throw new Error("Point is not on curve");
                const e = c(t);
                let i = r.sqrt(e);
    ...
    class I {
        ...
        static fromCompact(t) {
            ...
            return t = B("compactSignature", t, 2 * r), new I(w(t, 0, r), w(t, r, 2 * r))
        }
        static fromDER(t) {
            const {
                r: e,
    ...
    const S = {
        ...
        precompute: (t = 8, e = a.BASE) => (e._setWindowSize(t), e.multiply(BigInt(3)), e)
    };
    ...
    function L(t) {
        ...
        if (!(X <= t && t < H)) throw new Error(`bigint expected < 2^${e.nBitLength}`);
    ...
    function U(t, n, i = $) {
        ...
        null == h && (h = !0), t = B("msgHash", t), d && (t = B("prehashed msgHash", o(t)));
        const p = N(t),
            y = u(n),
            m = [L(y), L(p)];
        if (null != w) {
    ...
    return a.BASE._setWindowSize(8), {
        ...
        verify: function(t, r, n, i = z) {
            var o;
            const u = t;
            if (r = B("msgHash", r), n = B("publicKey", n), "strict" in i) throw new Error("options.strict was renamed to lowS");
            const {
                lowS: c,
                prehash: l
            } = i;
            let h, d;
            try {
                if ("string" == typeof u || u instanceof Uint8Array) try {
                    h = I.fromDER(u)
                } catch (x) {
                    if (!(x instanceof Q.Err)) throw x;
                    h = I.fromCompact(u)
                } else {
                    if ("object" != typeof u || "bigint" != typeof u.r || "bigint" != typeof u.s) throw new Error("PARSE"); {
                        const {
                            r: t,
                            s: e
                        } = u;
                ...
                if ("PARSE" === v.message) throw new Error("signature must be Signature instance, Uint8Array or hex string");
                return !1
            }
            ...
            const {
                r: g,
                s: w
            } = h, p = N(r), y = f(w), m = s(p * y), E = s(g * y), b = null == (o = a.BASE.multiplyAndAddUnsafe(d, m, E)) ? void 0 : o.toAffine();
            return !!b && s(b.x) === g
        },
        ProjectivePoint: a,
        Signature: I,
...
class it extends t {
    constructor(t, n) {
        super(), this.finished = !1, this.destroyed = !1, e(t);
        const i = r(n);
    ...
    digestInto(t) {
        n(this), i(t, this.outputLen), this.finished = !0, this.iHash.digestInto(t), this.oHash.update(t), this.oHash.digestInto(t), this.destroy()
    }
    digest() {
...
const ht = function(t, e, r = !1, n = {}) {
        if (t <= H) throw new Error(`Expected Field ORDER > 0, got ${t}`);
        const {
            nBitLength: i,
            nByteLength: o
        } = _(t, e);
        ...
        const s = C(t),
            f = Object.freeze({
                ...
                pow: (t, e) => function(t, e, r) {
                    if (r < H) throw new Error("Expected power > 0");
                    if (r === H) return t.ONE;
                    if (r === T) return e;
                    let n = t.ONE,
                        i = e;
                    for (; r > H;) r & T && (n = t.mul(n, i)), i = t.sqr(i), r >>= T;
                    return n
                }(f, t, e),
                div: (e, r) => F(e * j(r, t), t),
                sqrN: t => t * t,
        ...
        sqrt: function(t) {
            const e = ft,
                ...
                B = k(b, n, e) * u % e,
                x = k(B, ct, e);
            if (!ht.eql(ht.sqr(x), t)) throw new Error("Cannot find square root");
            return x
        }
    }),
    dt = function(t, e) {
        ...
        endo: {
            ...
            splitScalar: t => {
                ...
                const l = u > s,
                    h = c > s;
                if (l && (u = e - u), h && (c = e - c), u > s || c > s) throw new Error("splitScalar: Endomorphism failed, k=" + t);
                return {
                    k1neg: l,
                    k1: u,
                    k2neg: h,
